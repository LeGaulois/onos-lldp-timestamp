...
public class DefaultLink extends AbstractProjectableModel implements Link {
    ...
    public void getStackTrace(int number){
        StackTraceElement[] stackTraceElements = Thread.currentThread().getStackTrace();
        String str = "#"+Integer.toString(number)+">> ";

        for (int i = 1; i < stackTraceElements.length && i<=4; i++) {
            StackTraceElement stackTraceElement = stackTraceElements[i];
            str = str+"[ ClassName: "+stackTraceElement.getClassName() + "; Method "
        + stackTraceElement.getMethodName() +"; FileName: "+stackTraceElement.getFileName()+"; Line: "+stackTraceElement.getLineNumber()+"], "; 
        }
        log.info("{}", str);
    }
    ...
    protected DefaultLink(ProviderId providerId, ConnectPoint src, ConnectPoint dst,
                       Type type, long delay, State state, Annotations... annotations) {
        this(providerId, src, dst, type, state, false, annotations);
	    this.delay = delay;
    }
    ...
    private DefaultLink(ProviderId providerId, ConnectPoint src, ConnectPoint dst,
                           Type type, long delay, State state,
                           boolean isExpected, Annotations... annotations) {
        super(providerId, annotations);
        this.src = src;
        this.dst = dst;
