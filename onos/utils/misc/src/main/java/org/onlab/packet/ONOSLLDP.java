 ...
 */
 ...
 FLO
 ADD TO:
 onos/utils/misc/src/main/java/org/onlab/packet
 */
...
public class ONOSLLDP extends LLDP {
    ...
    public ONOSLLDP(byte... subtype) {
        ...
        opttlvs.putIfAbsent(TIMESTAMP_SUBTYPE, new LLDPOrganizationalTLV());
    ...
    public void setTimestamp() {
        LLDPOrganizationalTLV timestamptlv = opttlvs.get(TIMESTAMP_SUBTYPE);
        Timestamp timestamp = new Timestamp(System.currentTimeMillis());
        String str_timestamp = timestamp.toString();
        timestamptlv.setInfoString(str_timestamp);
        timestamptlv.setLength((short) (str_timestamp.length() + TIMESTAMP_LENGTH));
        timestamptlv.setSubType(TIMESTAMP_SUBTYPE);
        timestamptlv.setOUI(MacAddress.ONOS.oui());
    }
    ...
    public Timestamp getTimestampTLV() {
        for (LLDPTLV tlv : this.getOptionalTLVList()) {
            if (tlv.getType() == LLDPOrganizationalTLV.ORGANIZATIONAL_TLV_TYPE) {
                LLDPOrganizationalTLV orgTLV =  (LLDPOrganizationalTLV) tlv;
                if (orgTLV.getSubType() == TIMESTAMP_SUBTYPE) {
                    String str_timestamp = new String(orgTLV.getInfoString(),StandardCharsets.UTF_8);
                            
                    return Timestamp.valueOf(str_timestamp);
                }
            }
        }
        return null;
